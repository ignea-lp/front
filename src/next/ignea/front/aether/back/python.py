...
class CommonFileFold(AetherCommonFileFold):
    def fold_file(self, conditions: list[str]) -> str:
        return f"{GENERATED_MESSAGE}from ignea.front.common import IgneaConditions, IgneaCondition\n\n\nclass Conditions(IgneaConditions):\n    ZERO = 0{'\n    ' + '\n    '.join(conditions) if len(conditions) > 0 else ''}"

    def fold_condition(self, name: str) -> str:
        return f"{_escape_identifier(name)} = IgneaCondition()"
...
class SyntacticFileFold(AetherSyntacticFileFold):
    def fold_file(
        ...
        return f"{GENERATED_MESSAGE}from ignea.front.syntactic import ignea_selection, IgneaNonterminalType, IgneaParser, IgneaDerivationException\nfrom .common import Conditions\nfrom .lexical import *\n\n\n{'\n\n\n'.join(nonterminal_types)}\n\n\nclass Parser(IgneaParser):\n    NONTERMINAL_TYPES = [{', '.join(_escape_identifier(n) for n in nonterminal_type_names)}]"
